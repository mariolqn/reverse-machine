...
export function validateInputFile(filename: string): string {
  ...
  if (!isInWorkingDir && !isInUserHome) {
    throw new SecurityError('Path traversal detected - file must be within working directory or user home');
  }
  ...
  if (!ALLOWED_EXTENSIONS.includes(extension)) {
    throw new SecurityError(`Invalid file type. Allowed extensions: ${ALLOWED_EXTENSIONS.join(', ')}`);
  }
  
  // Check file size to prevent DoS
  const stats = fs.statSync(absolutePath);
  if (stats.size > MAX_FILE_SIZE) {
    throw new SecurityError(`File too large. Maximum size: ${MAX_FILE_SIZE / 1024 / 1024}MB`);
  }
...
export function validateOutputPath(outputDir: string): string {
  ...
  if (!resolved.startsWith(workingDir)) {
    throw new SecurityError('Output directory must be within working directory');
  }
  
  // Prevent dangerous directory names
  const dangerousPaths = ['/', '/etc', '/usr', '/var', '/bin', '/sbin', '/home'];
  if (dangerousPaths.some(dangerous => resolved.startsWith(dangerous))) {
    throw new SecurityError('Output directory not allowed in system directories');
  }
  ...
  try {
    ...
    throw new SecurityError(`Failed to create output directory: ${(error as Error).message}`);
  }
