...
export function geminiRename({
  ...
  return async (code: string): Promise<string> => {
    ...
    // Advanced models (Gemini 2.5+, 1.5 Pro) default to maximum quality mode
    ...
    if (shouldUseAdvanced) {
      ...
      return await processWithGemini25(apiKey, modelName, code);
    } else {
...
async function performQualityCheck(client: any, model: string, code: string): Promise<string> {
  ...
  try {
    const response = JSON.parse(result.response.text());
    SecureLogger.debug(`Quality score: ${response.qualityScore}`);
    ...
    return response.finalCode || code;
  } catch (error) {
