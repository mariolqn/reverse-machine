...
test.beforeEach(async () => {
  // Small file (1KB)
  await writeFile(SMALL_TEST_FILE, `
...
class f{constructor(g){this.h=g}}
  ...
  // Large file (100KB+)
  const largeCode = Array.from({ length: 2000 }, (_, i) => `
...
test("Performance: Small file processing time", async () => {
  const start = performance.now();
  
  await humanify("local", SMALL_TEST_FILE, "--outputDir", OUTPUT_DIR);
...
test("Performance: Large file processing time", async () => {
  const start = performance.now();
  
  await humanify("local", LARGE_TEST_FILE, "--outputDir", OUTPUT_DIR);
  
  const end = performance.now();
  const duration = end - start;
  
  // Large files should still be reasonable (under 2 minutes)
  assert(duration < 120000, `Large file took too long: ${duration}ms`);
  console.log(`Large file processing time: ${duration.toFixed(2)}ms`);
});

test("Performance: Multiple small files", async () => {
  const files = [];
  const promises = [];
  ...
  for (const file of files) {
    promises.push(humanify("local", file, "--outputDir", `${OUTPUT_DIR}-${file}`));
  }
...
test("Performance: Output file quality check", async () => {
  await humanify("local", LARGE_TEST_FILE, "--outputDir", OUTPUT_DIR);
  
  const outputContent = await readFile(`${OUTPUT_DIR}/deobfuscated.js`, "utf-8");
...
test("Performance: CLI startup time", async () => {
  ...
  assert(avgTime < 5000, `CLI startup too slow: ${avgTime.toFixed(2)}ms average`);
