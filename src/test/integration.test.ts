...
for(var n=0;n<10;n++){console.log(n)}
  ...
  const result = await humanify("local", testFile, "--outputDir", TEST_OUTPUT_DIR);
  
  assert(result.stdout.includes("Successfully") || result.stderr.includes("Successfully"), 
         "Should indicate successful processing");
  
  const outputContent = await readFile(`${TEST_OUTPUT_DIR}/deobfuscated.js`, "utf-8");
  
  // Verify output is longer and more readable
  assert(outputContent.length >= complexCode.length, "Output should be at least as long as input");
  assert(outputContent.includes("function"), "Should contain function declarations");
});

test("Integration: Multiple file processing workflow", async () => {
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    await humanify("local", inputFile, "--outputDir", outputDir);
    
    const outputContent = await readFile(`${outputDir}/deobfuscated.js`, "utf-8");
    assert(outputContent.includes("function"), `File ${i} should be processed correctly`);
  }
});

test("Integration: Error recovery and graceful degradation", async () => {
  ...
  try {
    await humanify("local", malformedFile, "--outputDir", TEST_OUTPUT_DIR);
...
test("Integration: File system edge cases", async () => {
  ...
  await humanify("local", unusualFile, "--outputDir", TEST_OUTPUT_DIR);
  
  const outputContent = await readFile(`${TEST_OUTPUT_DIR}/deobfuscated.js`, "utf-8");
  assert(outputContent.includes("function"), "Should handle unusual filenames");
});

test("Integration: Large batch processing", async () => {
  const batchSize = 10;
  const files = [];
...
function batch${i}(param) {
  ...
  for (const file of files) {
    ...
    await humanify("local", file, "--outputDir", outputDir);
    
    const outputContent = await readFile(`${outputDir}/deobfuscated.js`, "utf-8");
    assert(outputContent.includes("function"), "Each batch file should be processed");
  }
});
...
test("Integration: Verbose mode provides detailed output", async () => {
  ...
  const verboseResult = await humanify("local", testFile, "--verbose", "--outputDir", TEST_OUTPUT_DIR);
  const normalResult = await humanify("local", testFile, "--outputDir", `${TEST_OUTPUT_DIR}2`);
...
test("Integration: Security boundaries are respected", async () => {
...
function test() {
  ...
  await humanify("local", testFile, "--outputDir", TEST_OUTPUT_DIR);
  
  const outputContent = await readFile(`${TEST_OUTPUT_DIR}/deobfuscated.js`, "utf-8");
  
  // Verify no sensitive paths are mentioned in output
  const sensitivePatterns = ["/etc/passwd", "/usr/bin", "C:\\Windows\\System32"];
